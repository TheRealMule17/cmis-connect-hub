...
const SponsorShowcase = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("sponsor_profiles").select("*").order("tier", {
        ascending: true
      });
  ...
  const renderSponsorGrid = (sponsors: Sponsor[], tierName: string) => {
    ...
    return <div className="grid gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {sponsors.map(sponsor => <div key={sponsor.id} className="p-4 md:p-6 border border-border rounded-lg bg-card hover:shadow-lg transition-shadow duration-300 space-y-3">
            {sponsor.url || sponsor.website_url ? <a href={sponsor.url || sponsor.website_url || '#'} target="_blank" rel="noopener noreferrer" className="block">
                {sponsor.logo_url ? <div className="flex items-center justify-center h-16 md:h-20 mb-3">
                    <img src={sponsor.logo_url} alt={sponsor.company_name} className="max-h-full max-w-full object-contain" />
                  </div> : <div className="flex items-center justify-center h-16 md:h-20 mb-3">
            ...
            {sponsor.memberSince && <p className="text-sm text-muted-foreground">
                Member Since: {sponsor.memberSince}
                {sponsor.description === "Founding Member" && " (founding member)"}
              </p>}
            
            {sponsor.representative && <p className="text-sm">
